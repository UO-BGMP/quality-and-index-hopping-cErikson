...
if __name__ == "__main__":
    ...
    parser.add_argument("index_col", help="Column in Known indexes File that contains barcodes ", type=int)
...
with open(ARGS.known_file,'r') as kf:
    for line in kf:
        codes.add(line.split('\t')[ARGS.index_col].strip())
for c1 in codes:
    for c2 in codes:
        known.append((c1,c2))
known.append('unknown_index')
del codes
...
# open counts file
if os.path.isfile(ARGS.out_dir+'counts.json') is True:
    with open(ARGS.out_dir+'counts.json', 'r') as fp:
        counts = json.load(fp)
else:
    counts={}
...
for r1, i1, i2, r2 in zip(yield_fastq(ARGS.read1_file), yield_fastq(ARGS.index1_file), yield_fastq(ARGS.index2_file), yield_fastq(ARGS.read2_file)): # for each input fastq
        if all(x >= ARGS.cutoff for x in i1[2] and i2[2]): # if all the phred scores pass cutoff
            counts.setdefault(i1[1]+'_'+i2[1],{ARGS.cutoff:0}) # set default tag
            counts[i1[1]+'_'+i2[1]].setdefault(ARGS.cutoff,0) # Set default cutoff
            counts[i1[1]+'_'+i2[1]][ARGS.cutoff]+=1    # increment index count            
            for fh, data in zip(files[i1[1]+'_'+i2[1] if i1[1]+'_'+i2[1] in files else 'unknown_index'], [r1, i1, i2, r2]):   # for each output file and its respective input entry, and determine if known          
                fh.write('{0}_{1}\n{2}\n+\n{3}\n'.format(data[0].split()[0], i1[1]+'_'+i2[1], data[1], ''.join((chr(x+33) for x in data[2])))) # write the fastq

# write JSON 
with open(ARGS.out_dir+'counts.json', 'w') as fp:
